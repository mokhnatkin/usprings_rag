...
**В работе (MVP1):** многопользовательский портал - аутентификация, роли, логирование
вопросов-ответов, история, админка. Сделаны этапы 1-8: коллекции переехали в БД,
внутренние учётки + cookie-сессии + bootstrap super-admin, роли и доступ к коллекциям,
журнал `query_log` + обратная связь + история; загрузка PDF из UI с фоновой
индексацией и soft-delete (архивация) инструкций; справочники пользователей и
коллекций для super-admin и просмотр журнала вопросов-ответов. Осталось: калибровка
порога из UI, аналитика, финальная навигация, упаковка. Направления - в
