...
вопросов-ответов, история, админка. Сделаны этапы 1-6: коллекции переехали в БД,
внутренние учётки + cookie-сессии + bootstrap super-admin, роли и доступ к коллекциям,
журнал `query_log` + обратная связь + история. Направления - в
...
константа; задаётся в `src/usprings_rag/collection.py`). Прежние `recall@1 = 17/17`
