...
**В работе (MVP0):** база знаний разделена на **коллекции** по продуктам -
...
План работ - [`docs/MVP/MVP0/backlog.md`](docs/MVP/MVP0/backlog.md).

Прежние цифры качества (`recall@1 = 17/17` при `SIMILARITY_THRESHOLD=0.53`) получены
на тестовом корпусе из 8 инструкций, который удалён, - **как действующие их приводить
нельзя**: пороги калибруются заново, отдельно для каждой коллекции (порог стал
свойством коллекции, а не одной глобальной константой).
...
- **Калибруемые параметры** (`TOP_K`, `CHUNK_*`, `OPENROUTER_MODEL`, ...) - в `.env`,
  ...
  **Порог сходства - не в `.env`**: он свой у каждой коллекции и задаётся в
  справочнике `src/usprings_rag/collection.py`.
