...
# Латиницей намеренно: кириллический маркер модели искажают (gemma на A/B вернула
# «НCЕТ_ОТВЕТА» с латинской C - проверка не сработала бы, и служебное слово ушло бы
# пользователю). ASCII-токен воспроизводится надёжно.
...
- Отвечай по существу и пошагово, если инструкция описывает шаги. Не ограничивайся
  односложным «да/нет»: если инструкция подсказывает, что делать дальше - скажи это.
- Язык ответа - русский.
- Пиши обычным текстом, без markdown-разметки (никаких **, ##, `). Портал показывает
  ответ как есть - символы разметки будут видны пользователю. Нумерованные списки
  оформляй просто: «1. ...».
