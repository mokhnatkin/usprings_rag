...
- [x] **Первый CI-пайплайн на gtl зелёный** (2026-07-21, пайплайн 145 на `73d36fa`,
  430 c): `lint` 124 c - `ruff check .` чист; `test` 149 c - миграции 0001-0008 на
  postgres-сервисе, **125 passed, 0 skipped**; `build:images` 158 c - в registry
  `gtl.usteel.ru:5050/usprings/usprings_rag` теги `73d36fa8` и `latest`.
  До него 11 пайплайнов подряд были красными (с 2026-07-12) - все падали на `lint`,
  поэтому `test` и `build` не выполнялись ни разу. Починены две реальные причины:
  (1) `ruff` не проходил никогда - 7x `E402` (блок импортов в середине
  `tests/test_calibration.py`) и 2x `F401`; (2) каталога `docs/manuals` не было в git
  (`.gitignore` прятал только `*.pdf`, а пустой каталог git не хранит), а `api.py`
  монтирует его как статику **на импорте модуля** - в свежем клоне сбор 11 тестов
  падал с `RuntimeError: Directory 'docs/manuals' does not exist`. Тот же дефект
  сломал бы и деплой с нуля; на staging не проявился, т.к. корпус залили scp до
  первого старта. Лечение - `docs/manuals/.gitkeep`.
...
Выполнено 2026-07-21 (пайплайн 145 зелёный, см. «Статус выполнения»).

1. Артефакт 2 (`.gitlab-ci.yml`) + 3 (`ruff`) в `main` (шаг 0.4).
2. Пайплайн запускается сам на каждый push в `main` (`workflow.rules`); раннер
   у проекта есть. Registry включён, образ пушится с тегами `<short-sha>` и `latest`.
3. **Смотреть результат пайплайнов.** Красный `main` может стоять неделями незамеченным
   (так и вышло: 11 красных подряд). Диагностика без UI - GitLab API с PAT (scope
   `read_api`): `/projects/usprings%2Fusprings_rag/pipelines`, далее `/pipelines/<id>/jobs`
   и `/jobs/<id>/trace` за логом.
4. Типовые причины падения: `lint` - замечания `ruff` (гонять `uv run ruff check .`
   до пуша); `test` - нет БД (проверить alias `db` и `DATABASE_URL`) либо импорт
   `usprings_rag.api` требует каталогов, которых нет в свежем клоне.
